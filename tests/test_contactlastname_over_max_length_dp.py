...
"""
Test Run: ContactLastName Over Max Length (D/P Records)
Injects 26-character ContactLastName for D/P records to validate max length boundary.
"""


@allure.feature('ETL Validation')
@allure.story('Invalid Data Handling')
@allure.title('Test ContactLastName Over Max Length (D/P Records)')
class TestContactLastNameOverMaxLengthDP:

    @allure.description("""
    Test that pipeline rejects ContactLastName values over 25 chars for D/P records.

    Steps:
    1. Generate invalid parquet file
    2. Force OrganizationCode to D/P only
    3. Inject 26-char ContactLastName
    4. Upload/trigger ETL pipeline
    5. Verify invalid handling in validation output
    """)
    @allure.severity(allure.severity_level.CRITICAL)
    def test_contactlastname_over_max_length_dp(self):
        ...
        pipe_command = [
            ...
            "--dev2",
            "--rows", "25",
            "--test-name", "test_contactlastname_over_max_length_dp"
        ]

        with allure.step("Run ETL pipeline with ContactLastName > 25 chars for D/P records"):
