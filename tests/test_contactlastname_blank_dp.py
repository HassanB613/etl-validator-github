...
"""
Test Run: ContactLastName Blank (D/P Records)
Injects blank ContactLastName for D/P records to validate min length/required boundary.
"""


@allure.feature('ETL Validation')
@allure.story('Invalid Data Handling')
@allure.title('Test ContactLastName Blank (D/P Records)')
class TestContactLastNameBlankDP:

    @allure.description("""
    Test that pipeline rejects blank ContactLastName for D/P records.

    Steps:
    1. Generate invalid parquet file
    2. Force OrganizationCode to D/P only
    3. Inject blank ContactLastName
    4. Upload/trigger ETL pipeline
    5. Verify invalid handling in validation output
    """)
    @allure.severity(allure.severity_level.CRITICAL)
    def test_contactlastname_blank_dp(self):
        ...
        pipe_command = [
            ...
            "--dev2",
            "--rows", "25",
            "--test-name", "test_contactlastname_blank_dp"
        ]

        with allure.step("Run ETL pipeline with blank ContactLastName for D/P records"):
