...
"""
Test Run: Special Characters in AccountType Column
Injects special characters '$%$%$%$%' into the AccountType column to test ETL validation.
"""

@allure.feature('ETL Validation')
@allure.story('Invalid Data Handling')
@allure.title('Test AccountType Column with Special Characters ($%$%$%$%)')
class TestAccountTypeInvalidSpecialChar:
    
    @allure.description("""
    Test that pipeline rejects special characters '$%$%$%$%' in AccountType column.
    
    Steps:
    1. Generate invalid parquet file with special characters '$%$%$%$%' in AccountType
    ...
    def test_accounttype_invalid_special_char(self):
        """
        Test that pipeline rejects special characters '$%$%$%$%' in AccountType column.
        ...
        # Run the pipeline with special characters '$%$%$%$%' injected into AccountType column
        pipe_command = [
            sys.executable, pipeline_path,
            "--invalid-values", "AccountType:$%$%$%$%",
            "--dev2",
            "--rows", "25",
            "--test-name", "test_accounttype_invalid_special_char"
        ]
        
        with allure.step("Inject special characters '$%$%$%$%' into AccountType column"):
            print("=" * 60)
            print("TEST: Special Characters '$%$%$%$%' in AccountType Column")
            print("=" * 60)
            print(f"Injecting: $%$%$%$% into AccountType column")
